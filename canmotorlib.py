...
class CanMotorController:
    ...
    def __init__(self, can_socket="can1", motor_id=0x01, motor_type="AK80_6_V1p1", socket_timeout=0.05):
        """
        ...
        #ADDED EXPERIMENTAL
        elif motor_type == "GIM8108":
            self.motorParams = GIM8108_PARAMS
        ...
        # if not self.can_socket in CanMotorController.can_busses_used:
        if 1==1:

            # create a raw socket and bind it to the given CAN interface
            try:
                CanMotorController.can_busses_used.append(can_socket)

                # Create a CAN filter to only accept messages with the specified motor_id
                can_filter = struct.pack("=LL", self.motor_id, socket.CAN_EFF_MASK)

                self.motor_socket = socket.socket(socket.AF_CAN, socket.SOCK_RAW, socket.CAN_RAW)

                # Apply the CAN filter
                self.motor_socket.setsockopt(socket.SOL_CAN_RAW, socket.CAN_RAW_FILTER, can_filter)
                ...
                print("Error:", e)
        elif self.can_socket in CanMotorController.can_busses_used:
            print("Socket already available. Using:  ", self.motor_socket)
    ...
    def _send_can_frame(self, data):
        ...
        can_msg = struct.pack(can_frame_fmt_send, self.motor_id, can_dlc, data) #note that hex will get converted to ascii but no errors result
    ...
    def _send_can_frame_ADMIN(self, can_msg):
        """
        Send raw CAN data frame (in bytes) to the motor.
        """
        print(f"sending CAN message:{can_msg}")
    ...
    def _recv_can_frame(self):
        ...
        try:
            ...
            # while can_id != self.motor_id:

            frame, addr = self.motor_socket.recvfrom(recvBytes)
            can_id, can_dlc, data = struct.unpack(can_frame_fmt_recv, frame)

            if can_id == self.motor_id:
                print(f"Received message from CAN ID {hex(can_id)} with data: {data}")
            else:
                print(f"Ignoring message from CAN ID {hex(can_id)}")
...
if __name__ == "__main__":
    ...
    posdeg,veldeg,curr=M13.M.send_deg_command(15, 0, 15, 1, 0)
    print(f"Position: {posdeg} deg \n Velocity: {veldeg} deg/s \n Current: {curr} amps")
    posdeg,veldeg,curr=M15.M.send_deg_command(15, 0, 15, 1, 0)
    print(f"Position: {posdeg} deg \n Velocity: {veldeg} deg/s \n Current: {curr} amps")
    time.sleep(t)
    posdeg,veldeg,curr=M13.M.send_deg_command(0, 0, 15, 1, 0)
    print(f"Position: {posdeg} deg \n Velocity: {veldeg} deg/s \n Current: {curr} amps")
    posdeg,veldeg,curr=M15.M.send_deg_command(0, 0, 15, 1, 0)
    print(f"Position: {posdeg} deg \n Velocity: {veldeg} deg/s \n Current: {curr} amps")
    ...
    # M1 = CanMotorController("can1",15,"GIM8108")



    # M1.enable_motor()

    # posdeg,veldeg,curr=M1.send_deg_command(0, 0, 1, 1, 0)
    # # M1.enable_motor()
    # time.sleep(t)
    # posdeg,veldeg,curr=M1.send_deg_command(10, 0, 15, 1, 0)
    # print(f"Position: {posdeg} deg \n Velocity: {veldeg} deg/s \n Current: {curr} amps")
    # time.sleep(t)
    # posdeg,veldeg,curr=M1.send_deg_command(-10, 0, 15, 1, 0)
    # print(f"Position: {posdeg} deg \n Velocity: {veldeg} deg/s \n Current: {curr} amps")
    # # time.sleep(t)
    # # posdeg,veldeg,curr=M1.send_deg_command(-180, 0, 1, 1, 0)
    # # print(f"Position: {posdeg} deg \n Velocity: {veldeg} deg/s \n Current: {curr} amps")
    # # time.sleep(t)
    # # posdeg,veldeg,curr=M1.send_deg_command(0, 0, 1, 1, 0)
    # # print(f"Position: {posdeg} deg \n Velocity: {veldeg} deg/s \n Current: {curr} amps")
    ...
    # positionRawValue, velocityRawValue, currentRawValue = M1.decode_motor_status([0, 0, 0, 0, 0, 0])
    # print(positionRawValue, velocityRawValue, currentRawValue)



    # #def _send_raw_command(self, p_des, v_des, kp, kd, tau_ff):
    # print(f"Moving motor M1 with CAN command")
    # data = M1._send_raw_command(0,0,1,5,0)

    # #RAW CAN data in hex
    # print(f"Data Returned:{data}")

    # #Convert hex to bit mapped (16 or 12 bit) data from motor parameters
    # positionRawValue, velocityRawValue, currentRawValue = M1.decode_motor_status(data)
    # print(f"Raw Position: {positionRawValue} \n Raw Velocity: {velocityRawValue} \n Raw Current: {currentRawValue}")

    # #Convert Data to Rads
    # physicalPositionRad, physicalVelocityRad, physicalCurrent=M1.convert_raw_to_physical_rad(positionRawValue, velocityRawValue, currentRawValue)
    # print(f"Position: {physicalPositionRad} rads \n Velocity: {physicalVelocityRad} rads/s \n Current: {physicalCurrent} amps")
    

    # #Deg move command
    # #def send_deg_command(self, p_des_deg, v_des_deg, kp, kd, tau_ff)
    # posdeg,veldeg,curr=M1.send_deg_command(15, 0, 1, 1, 0)
    # time.sleep(1)
    # print(f"Position: {posdeg} deg \n Velocity: {veldeg} deg/s \n Current: {curr} amps")
    # posdeg,veldeg,curr=M1.send_deg_command(-15, 0, 1, 1, 0)
    # time.sleep(1)
    # print(f"Position: {posdeg} deg \n Velocity: {veldeg} deg/s \n Current: {curr} amps")
    # posdeg,veldeg,curr=M1.send_deg_command(0, 0, 3, 1, 0)
    # time.sleep(1)
    # print(f"Position: {posdeg} deg \n Velocity: {veldeg} deg/s \n Current: {curr} amps")
    # posdeg,veldeg,curr=M1.send_deg_command(365, 0, 1, 1, 0)
    # time.sleep(1)
    # print(f"Position: {posdeg} deg \n Velocity: {veldeg} deg/s \n Current: {curr} amps")
    

    # #print(f"CAN ID: {can_id} \n CAN_data length:{can_dlc} \n Data:{data} \n")
